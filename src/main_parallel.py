...
def main():
    ...
    # Launch chain of subproceses: 1 slicer + 3 presorter + 1 consolidator
    ...
    p3 = subprocess.Popen([sys.executable,"main_consolidator.py", 
               ...
               "-of", args.output_file])
    time.sleep(3)
    p22 = subprocess.Popen([sys.executable,"main_presorter.py", 
               "-n", str(args.nbig), "-if", args.input_file, "-of", "dummy"])
    time.sleep(3)
    p23 = subprocess.Popen([sys.executable,"main_presorter.py", 
               "-n", str(args.nbig), "-if", args.input_file, "-of", "dummy"])
