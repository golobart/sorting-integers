...
class PyArrayPreall(Selector):
    ...
    Use of array.insert to store values in topn store (topn is preallocated at full capacity)
    Note:
    ...
    def __init__(self, args, conf):
        ...
        """
        super(PyArrayPreall, self).__init__(args, conf)
        self.topn = arr.array("Q") # the store, array of 8 bytes elements, unsigned long long
                                  
        f = open("../data/in/init_store.txt", "rb")
        self.topn.fromfile(f, args.nbig) # preallocate memory at max value
        f.close()
        #self.v = memoryview(self.topn)
